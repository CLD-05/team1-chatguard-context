...
8. **금칙어 시드·동기화(D42·D51)**: ① *시드(DB 1회)* — chat-server 시작 시 Flyway V2 (`V2__InsertInitialBannedWords`)가 S3(`AWS_S3_BUCKET`/`AWS_S3_KEY`)의 `banned_words.txt`를 읽어 `banned_words`에 `INSERT IGNORE` 적재. `flyway_schema_history` 기록 → 파드 부팅마다 재실행 안 함(동시 부팅은 Flyway 락으로 1개만 적용). S3 미가독 시 classpath 더미 4단어로 안전 강하(stderr 로그만, 메트릭 부재 — 보강 후보 D51). ② *캐시 로드(파드별)* — 각 chat-server 파드 부팅 시 `banned_words` DB → 인메모리 캐시. `config:banned-words` 신호 수신 시 DB 재로드. ※ 시드가 `INSERT IGNORE`라 admin 런타임 CRUD(D46)는 정상 재시작 시 보존(V2 미재실행). 단 DB를 새로 생성/초기화하면 V2 재시드로 admin이 지운 seed 단어 부활 (dev 일일 destroy 맥락에선 무해).
