...
>
> **destroy 안전 절차 반영(2026-06-23): 야간 차단 중 destroy 중단 사고로 LBC 생성물(NLB·SG)이 `Team` 태그 없이 orphan으로 남아 삭제 정책에 막힌 현상 실증 → ① destroy 전 k8s 워크로드 선회수 절차 + ② LBC LB에 팀 태그 자동 부착 어노테이션을 정함(결정 기록 D49). 앱 본문(A~E) 무영향 — infra 운영/소유경계(CLAUDE.md §4) 결정. 상세 경위는 학습 문서 `2026-06-23 destroy 사고 회고`. 🟢 결정 확정·구현 예정(config·infra PR).**
>
> **WS 연결 cap·close code 1013 반영(2026-06-24): app PR #57(D18 파드당 연결 cap 구현 + AI 리뷰)에서 드러난 계약 보강 → cap 거부 close code=`1013`(Try Again Later), 신규 env `WS_CONNECTION_CAP`(기본 200)을 SSoT에 반영(결정 기록 D50, D29 보강). A-1 프론트 close code 분기·A-3 close code 표("이 둘만"→"이 셋만")·A-5 env 계약 갱신. CHAT_FROZEN은 이미 A-3 에러표에 존재(D45 반영분)라 변경 없음 — app 레포 `_context` 사본 동기화만 확인 권고. 코드가 먼저 머지돼 문서가 뒤따른 형태 → 동기화 완료. 🟢 구현 완료(app)·문서 반영.**
> **금칙어 시드 출처 명문화(2026-06-26): D42 보강 — 1차 키워드의 *초기 시드*는 S3(`team1-dev-chatguard-migration`/`banned_words.txt`)를 chat-server 시작 시 Flyway V2가 읽어 `banned_words` DB에 `INSERT IGNORE`(DB 수명당 1회). 수동 주입·git 미커밋(검열 우회 방지). DB 런타임 진실원·캐시·무효화는 D42 불변. S3 미가독 시 더미 4단어 fallback. S3 접근=IRSA(C-1). env `AWS_S3_BUCKET`/`AWS_S3_KEY`/`AWS_REGION`(ConfigMap, 3곳 일치). 설계≠구현 갭 해소 (결정 기록 D51). A-2·A-4·A-5 갱신. 🟢 구현 검증 완료·문서 반영.**
> **어드민 행동 감사 로그 도입(2026-06-27): 어드민이 `banned_words` CRUD(D46)·freeze(D45) 권한을 악용·오용했을 때 추적할 감사 자산. Spring AOP + `@AdminLog` + `SecurityContext` 기반 비침습 수집 → `admin_audit_logs` DB 영속화. 조회는 `GET /api/admin/audit-logs`(role=ADMIN). A-1 책임·A-2 신규 테이블·A-3 어드민 API 갱신 (결정 기록 D52). 🟢 설계 확정·구현 예정. (동료 제안의 "D47"은 통신 경로 정정에 이미 사용 중이라 D52로 발급 — 번호 충돌 교정.)**
>
> **주말 집중 세션 반영(2026-06-28): 인스턴스 t→m7g(arm64, D9 일부 번복)·prod 실 HA화·관측성 완성/SRE 갭·stateful state 레이어 분리 결정(결정 기록 D53~D56). C-3 인스턴스 표·D9 갱신. arm 전환은 torch aarch64 휠 PyPI 실존 + 로컬 buildx 빌드(88초, torch·모델 베이크 통과) 실증. 개인계정 이식안은 검토 후 폐기(이식 품 과다 — 권한경계 12 role·state 버킷·OIDC 등) → infra 레포 직접 수정 후 월요일 부트캠프 apply로 오류 수렴. 정합성 점검(2026-06-28)이 드러낸 코드 버그·드리프트(#1~#12)는 설계 결정 아님 → 해당 infra/config PR 설명에 기록. 🟢 설계 확정·구현 예정(infra·config PR).**
>
> **worker 장애대응 구조 반영(2026-06-29): AI worker에 processing queue + retry + DLQ + startup recovery 도입(D11 보강 — SIGKILL/OOM/crash 시 in-flight 유실 차단). A-5 env 6개(`PROCESSING_QUEUE_KEY`·`DLQ_QUEUE_KEY`·`MAX_RETRY_COUNT`·`PROCESSING_TIMEOUT_SECONDS`·`RECOVER_PROCESSING_ON_STARTUP`·`WORKER_ID`)·B-2 메트릭 3종(`moderation_retries_total`·`moderation_dlq_total`·`moderation_recovered_processing_total`) 추가 (결정 기록 D57). app PR 머지·문서 반영. config ConfigMap에 env 6개 반영은 후속(D24 정합).**
...
| 컴포넌트            | 기술                           | 책임                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ------------------- | ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Frontend            | React                          | 채팅 UI, WebSocket 클라이언트, 메시지 렌더링, `moderation.hide` 수신 시 해당 메시지 블러/삭제, REST로 방·히스토리 조회, `error` 이벤트 처리(차단됨 표시·재시도 안내)와 close code 분기(1001=재연결 / 1013=jittered backoff 후 재연결 / 1008=중단)(D29·D30·D50). **배포 = 클러스터 파드(nginx 정적 서빙, D37). `/api`·`/ws`는 nginx를 거치지 않고 ALB Ingress가 Chat Server로 직접 라우팅(D47, D39 개정) — nginx 리버스 프록시 제거, 프론트는 상대경로·`wss://${location.host}/ws` 무변경.** **[Week4]** WebSocket 자동 재연결 + jittered backoff, 재접속 시 최근 윈도우 캐치업 병합(검열 상태 `message.status` 단일화) **[고도화]** 어드민 페이지(대시보드·금칙어·로그 탭, 진입 가드 `role=ADMIN`), 참여자 수·목록(`presence.update`), freeze 배너(`room.freeze`), 동영상 독립 재생(백엔드 무관) (D41·D44·D45) |
| Chat Server         | Spring Boot (WebSocket + REST) | 연결 수락·유지, 인증, 채팅 수신, **1차 키워드 검열(동기)**, message id 발급, Redis 채널 publish/subscribe, **구독 수신 시** 자기 클라이언트에 push(로컬 직접 push 안 함 — A-4 step3), 의심 메시지 큐 적재, MySQL 영속화, 메트릭 노출. **[Week4]** graceful drain(preStop sleep → SIGTERM 1001 close), 새 연결 분산(ALB LOR) + pod당 연결 cap(초과 시 `1013` close — D50) **[고도화]** 금칙어 DB+인메모리 캐시+`config:banned-words` 무효화 구독, 어드민 인가(role)·`/api/admin/*`(403 FORBIDDEN), presence(Redis Set 갱신+`presence.update`), freeze(상태 캐시+`chat.send` 집행), **[감사] `@AdminLog` AOP 기반 비침습적 어드민 행동 로그(Audit Log) 수집·영속화(D52)** (D41·D42·D44·D45·D52)                                                                                                                                                                                                             |
...
admin_audit_logs                       -- 어드민 행동 감사 로그 (D52)
  id           BIGINT       PK          -- 로그 고유 식별자 (Auto Increment)
  admin_id     VARCHAR(255) NOT NULL    -- 행위 수행 어드민 식별자(username). admin_id 형식은 구현 시 확정 — username vs user_id(JWT sub) 트레이드오프(D52 보강 후보)
  action       VARCHAR(100) NOT NULL    -- 행동 유형 (예: CREATE_KWD, DELETE_KWD, FREEZE_ROOM)
  resource_id  VARCHAR(255)             -- 변경 대상 리소스 ID/Key (방 번호, 금칙어 ID 등)
  description  TEXT                     -- 상세 컨텍스트 (Before/After 스냅샷 등)
  created_at   DATETIME     NOT NULL    -- 로그 생성 시각 (UTC — 시간 규약)
...
- 🟢 **금칙어(1차 키워드) = `banned_words` DB 진실원 + Chat Server 파드 인메모리 캐시 + `config:banned-words` Redis pub/sub 무효화.** 검사는 캐시만 읽어 **메시지당 DB 조회 없음**(전송 SLO 보호). 운영자가 어드민 페이지로 런타임 CRUD. `.env` 방식 폐지(EKS 미도달 버그 해소). 전역(모든 방·유저 공통). 매칭=대소문자 정규화 substring contains. (결정 기록 D42) 초기 목록(시드)은 S3 버킷 `team1-dev-chatguard-migration`의 `banned_words.txt`를 chat-server 시작 시 Flyway V2 마이그레이션이 `INSERT IGNORE`로 적재(DB 수명당 1회, `UNIQUE(word)`로 멱등 — 수동 주입·git 미커밋). S3 미가독 시 classpath 더미 4단어 fallback. (결정 기록 D51)
...
| GET    | /api/admin/audit-logs?admin_id={id}&before={id}&limit=50                 | 어드민 행동 감사 로그 조회(최신 50건, 커서 페이징). 🟢 설계 확정·구현 예정 — 상위 운영자·보안팀이 웹 화면으로 조회 (D52)                  |
...
WS — close code (아래 셋만 사용):

| code | 의미                             | 클라 처리                            |
| ---- | -------------------------------- | ------------------------------------ |
| 1001 | 서버 드레인(scale-in·배포 — D15) | 즉시 자동 재연결(D14)                |
| 1008 | 프로토콜·인증 위반               | 재연결하지 않고 로그인 화면으로      |
| 1013 | 파드 연결 cap 초과(D18·D50)      | jittered backoff 후 자동 재연결(D14) |
...
8. **금칙어 시드·동기화(D42·D51)**: ① _시드(DB 1회)_ — chat-server 시작 시 Flyway V2 (`V2__InsertInitialBannedWords`)가 S3(`AWS_S3_BUCKET`/`AWS_S3_KEY`)의 `banned_words.txt`를 읽어 `banned_words`에 `INSERT IGNORE` 적재. `flyway_schema_history` 기록 → 파드 부팅마다 재실행 안 함(동시 부팅은 Flyway 락으로 1개만 적용). S3 미가독 시 classpath 더미 4단어로 안전 강하(stderr 로그만, 메트릭 부재 — 보강 후보 D51). ② _캐시 로드(파드별)_ — 각 chat-server 파드 부팅 시 `banned_words` DB → 인메모리 캐시. `config:banned-words` 신호 수신 시 DB 재로드. ※ 시드가 `INSERT IGNORE`라 admin 런타임 CRUD(D46)는 정상 재시작 시 보존(V2 미재실행). 단 DB를 새로 생성/초기화하면 V2 재시드로 admin이 지운 seed 단어 부활 (dev 일일 destroy 맥락에선 무해).
...
Chat Server
  ...
  WS_CONNECTION_CAP=200,
  AWS_S3_BUCKET, AWS_S3_KEY=banned_words.txt, AWS_REGION
Moderation Worker
  DB_URL, DB_USER, DB_PASSWORD(secret),
  REDIS_HOST, REDIS_PORT,
  MOD_QUEUE_KEY=mod:queue, MODEL_VERSION,
  PROCESSING_QUEUE_KEY=mod:queue:processing, DLQ_QUEUE_KEY=mod:queue:dlq,
  MAX_RETRY_COUNT=3, PROCESSING_TIMEOUT_SECONDS=300,
  RECOVER_PROCESSING_ON_STARTUP=true, WORKER_ID=(hostname)
```

- 큐 키(`mod:queue`)·채널 패턴(`room:{id}`)은 A-3 확정값과 동일. 시크릿 원문은 Secrets Manager에만(코드·state 금지, CLAUDE.md §5).
- **worker 장애대응 env 추가(D57)**: `PROCESSING_QUEUE_KEY`(처리중 큐)·`DLQ_QUEUE_KEY`(데드레터)·`MAX_RETRY_COUNT`·`PROCESSING_TIMEOUT_SECONDS`(stale 판정)·`RECOVER_PROCESSING_ON_STARTUP`·`WORKER_ID`(processing 항목 소유자 식별, 기본 hostname). 전부 **비밀 아님·git 안정값**(configmap). 코드에 기본값이 있어 config 미반영이라도 동작하나, D24(app↔config 글자단위 일치)상 **config ConfigMap에도 동일 키 반영 권장**(미반영 시 운영자가 값 조정 불가). 큐 키 prefix(`mod:queue:*`)는 `MOD_QUEUE_KEY`에서 파생.
...
- **WS 연결 cap 추가(D18·D50)**: `WS_CONNECTION_CAP`(기본 `200`) 신규 env — 비밀 아님·git 안정값(configmap에 둠). 부하테스트 후 수치 재확정(D10·D18 연계). cap 초과 거부 close code=`1013`(A-3). app `@Value`·config configmap·(필요 시) infra 3곳 키 일치(D24·D36 정신).
- **금칙어 시드 S3 추가(D51)**: `AWS_S3_BUCKET`·`AWS_S3_KEY`·`AWS_REGION` 3키 신규 — ConfigMap(비밀 아님), `envFrom configMapRef`로 주입(`chat-server.yaml`). 앱 env명(`System.getenv`)↔config `base/configmap.yaml`↔infra 버킷명(`main.tf`) 3곳 글자 단위 일치(D24·D36). S3 접근은 IRSA(SA `team1-dev-api-server-sa`/role `team1-dev-chat-server-s3-role`, `DefaultCredentialsProvider`) — 정적 키 없음. 시드 본문(`banned_words.txt`)은 git·state 미커밋(수동 주입 — 금칙어 노출 회피, D51).
...
- `moderation_retries_total` (counter) — 일시 실패 후 재적재 횟수 (장애대응, D57)
- `moderation_dlq_total` (counter) — 최대 재시도 초과로 DLQ 이동 횟수 (D57)
- `moderation_recovered_processing_total` (counter) — startup 시 stale processing job 복구 수 (D57)
...
| EKS 노드                | 2 × m7g.large (arm64)   | 3 × m7g.large (arm64)  |
...
| Redis 노드/failover     | 1노드, failover 없음    | 2노드+자동 failover+multi_az (D54) |
| 관측성 retention        | 7일                     | 30일+                  |
| 알람 채널               | dev                     | prod                   |

- 🟢 prod = **HA 구조(Multi-AZ RDS·다중 NAT·노드 AZ 분산)는 유지, 인스턴스 크기는 최소.** (결정 기록 D9)
- 🟢 **노드 인스턴스 = m7g.large(arm64)** — t의 CPU 크레딧이 부하 측정을 오염시켜 m계열로 전환(D9 일부 번복, 결정 기록 D53). `ami_type=AL2023_ARM_64_STANDARD` 동반 필수(arm 인스턴스+x86 AMI=노드그룹 생성 실패). c7g(compute)는 측정 후보(stretch).
- ★ **RDS/Redis 인스턴스 family는 t4g 유지**(D53). 2차 부하 병목은 DB **커넥션 수**(85 천장)지 CPU 크레딧이 아니었고, m-family는 micro/small이 없어 .large로 격상 시 비용 급증. 3차 부하에서 DB CPU 크레딧 스로틀이 실제 관측되면 그때 db.m 격상 재검토. (노드와 달리 RDS/Redis는 이미지 미탑재라 arm/x86 무관 — t4g가 이미 arm)
- ★ **prod AZ=3·NAT 다중·RDS Multi-AZ·Redis failover는 본 표의 설계값이 옳으나, 모듈이 AZ 2·NAT 1·Redis 단일노드를 하드코딩**해 미구현 상태였음(정합성 점검 2026-06-28) → **D54에서 변수화로 실구현**. FIS 장애주입 데모의 전제.
...
| D9  | prod 인프라 사양          | ~~HA 구조 + 최소 인스턴스(노드 t3.large×3, db.t4g.small Multi-AZ)~~ **→ D53으로 일부 번복: 노드 m7g.large(arm64). HA 구조·DB 사양은 D54로 실구현 보강**                                                   | 가이드라인 예시(m6i.large 등) 그대로                                    | (당초) 데모 목적이라 과한 사양 불필요. 평가는 설계(HA·변수화)지 용량이 아님. **(번복) 부하 측정 정확도를 위해 t의 CPU 크레딧 오염 제거 필요 → m7g(arm), 상세 D53. prod HA는 "구조 설계"는 옳았으나 코드 미구현이라 D54로 실구현**                                              |
...
| D29 | 에러 계약(REST·WS)        | REST 공통 에러 봉투 `{error:{code,message}}` + 코드표(A-3). WS 핸드셰이크 실패=업그레이드 거부(HTTP 401/404), 연결 후=in-band `error` 이벤트(연결 유지), close code는 **1001·1008·1013만**(1013=cap 초과 추가는 D50). `chat.send` 성공 ack 없음(self-echo가 ack — D23), content **최대 500자**. 연결 중 토큰 만료는 무시(신규 연결만 검증) | 루트 평탄화 / 성공 ack 추가 / 만료 시 강제 close                                          | 에러 포맷·close code 미정의면 담당자별 구현이 갈라짐. ack는 self-echo와 중복. 연결 중 재인증은 v1 비범위(D4와 일관)                                                                            |
...
| D52 | 어드민 행동 감사 로깅 체계 | Spring AOP + 커스텀 어노테이션(`@AdminLog`) + `SecurityContext` 기반 비침습적 수집 → `admin_audit_logs` DB 영속화. 조회는 `GET /api/admin/audit-logs`(role=ADMIN, D46 인가 일관). 🟢 설계 확정·구현 예정 | 컨트롤러/서비스 단 수동 로깅 코드 작성 / 인프라(AWS CloudTrail 등) 의존 | 어드민 행동 로그는 비즈니스 로직과 결합도가 낮아야 하므로 AOP 횡단 관심사 처리가 정석. `SecurityContextHolder` 연동으로 `admin_id` 추출 자동화해 개발자 실수 배제. 도메인 감사 자산이라 DB 영속화가 config 결합 회피에 유리(D41·D51 정신 일관). 어드민이 `banned_words` CRUD(D46) 권한을 악용·오용할 때 추적용 감사 자산. | 🟢   |
...
### 결정 기록 — destroy 안전 절차 (2026-06-23 추가)

> 야간 접근 차단(매일 18:00, CLAUDE.md §0) 중 `terraform destroy`가 중단된 사고에서 실증된 운영 결정. 앱 설계(A~E)는 무영향 — Terraform↔GitOps 소유 경계(CLAUDE.md §4)와 공유계정 가드레일(§1) 영역. 상세 경위·복구 절차는 학습 문서 `2026-06-23 destroy 사고 회고` 참조.
> 사고 요지: LBC가 **런타임에** 만든 리소스(ArgoCD용 NLB·LBC SG)는 Terraform `default_tags`가 안 타 `Team` 태그가 비고, destroy가 중간에 끊겨 LBC 자가 회수가 안 일어나자 orphan으로 남았다. 이 orphan은 ENI로 서브넷을, SG로 VPC를 붙잡아 삭제를 막았고, `Team` 태그 부재로 `DenyOtherTeamResources-team1` 정책에 의해 수동 삭제마저 거부됐다(태그 부착 후에야 삭제 가능).

| ID  | 결정                                               | 채택                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 검토한 대안                                                                                                                     | 근거                                                                                                                                                                                                                                                                                                                                                                        | 상태                   |
| --- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| D49 | destroy 전 워크로드 선회수 + LBC LB 팀 태그 자동화 | **예방(평상시)**: destroy **전에** k8s 오브젝트 선삭제로 LBC가 LB·SG를 자가 회수하게 함 — `kubectl delete ingress --all -A` + **LoadBalancer 타입 Service 삭제**(ArgoCD 해당, ingress 삭제만으론 안 잡힘) → `aws elbv2 describe-load-balancers` 빈 것 확인 후 `terraform destroy`. **안전망(사고 시)**: LBC가 만드는 LB에 팀 표준 태그를 자동 부착(`service.beta.kubernetes.io/aws-load-balancer-additional-resource-tags: "Team=team1,Environment={env},Project=chatguard"`) → orphan으로 남아도 `Team` 태그가 있어 정책에 안 막히고 즉시 삭제 가능. 둘 다 채택(예방+안전망 이중화). | LBC 생성물도 `default_tags`로 태그된다고 가정(불가 — 다른 시스템) / 매번 orphan 수동 정리만 / destroy 순서를 사람 기억에만 의존 | `default_tags`는 Terraform 생성물에만 적용 → LBC 런타임 생성물엔 미적용(이번 사고의 근본). 예방만 하면 destroy 중단 시 또 orphan(태그 없음)이 남고, 안전망만 하면 매번 수동 정리 필요 → 둘을 함께 둬야 "정상 destroy는 자동 회수, 중단 사고는 태그 덕에 즉시 복구"가 성립. 18:00 마감 임박 시 destroy 미착수(EKS/VPC 삭제 10~20분, 중단 시 state 꼬임)는 운영 수칙으로 병기 | 🟢 결정 확정·구현 예정 |

---

### 결정 기록 — WS 연결 cap · close code 1013 (2026-06-24 추가, D29 보강)

> PR #57(app, hochang1)에서 D18 파드당 연결 cap을 구현하며 AI 리뷰로 드러난 계약 보강. cap 거부 close code와 신규 env 키를 SSoT에 못 박는다. 코드가 먼저 머지(2026-06-24)돼 문서가 뒤따른 형태이므로 빠른 동기화가 목적. A-1·A-3·A-5 갱신. (PR은 presence.update 이중 전송 버그 수정·`leave()` 멱등화도 포함하나 이는 버그 수정이라 결정 기록 대상 아님.)

| ID  | 결정                          | 채택                                                                                                                                                                                                                                                                                                                       | 검토한 대안                                                        | 근거                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | 상태                     |
| --- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------ |
| D50 | cap 거부 close code · cap env | 파드 연결 cap(D18) 초과 거부 시 **`1013`(Try Again Later)** close → 클라는 jittered backoff 후 자동 재연결(D14). A-3 close code 표를 "이 둘만"→"이 셋만"으로 확장(1001·1008·1013). `WS_CONNECTION_CAP`(기본 `200`)을 A-5 env 계약에 추가(비밀 아님·configmap·부하테스트 후 재확정). A-1 프론트 close code 분기에 1013 반영 | ① 1008 유지(인증 위반 코드 재사용) ② 핸드셰이크 단계 HTTP 503 거부 | 1008은 A-3에서 "재연결 없이 로그인 화면" → 정상 인증된 유저가 cap 거부 시 로그아웃 화면을 보는 잘못된 UX. cap 초과는 인증 문제가 아니라 **일시적 과부하**라 `1013`(IANA 등록 close code, "서버 과부하로 종료, 잠시 후 재시도")이 의미상 정확. 구현이 `afterConnectionEstablished`(업그레이드 **후**)에서 cap을 검사하므로 HTTP 503(핸드셰이크 거부)이 아닌 WS close가 자연스러움. cap은 분산 수단 아닌 OOM 천장(D18)이라 평상시 도달이 드묾 — 1013은 가장자리 안전망. env는 git 안정값이라 A-5에 명문화(D24) | 🟢 구현 완료(app PR #57) |

### 결정 기록 — 금칙어 시드 출처 (2026-06-26 추가, D42 보강)

> 금칙어 S3 시드 구현을 Plan Mode read로 코드 검증(2026-06-26)해 D42를 보강. 설계 의도("금칙어는 git에 안 올린다")가 D42 본문에 빠져 있던 갭을 해소. A-2·A-4·A-5 반영.

| ID  | 결정                        | 채택                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 검토한 대안                                                                                      | 근거                                                                                                                                                                                                                                                     | 상태                                                                                                                                                                         |
| --- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D51 | 금칙어 시드 출처 (D42 보강) | S3 버킷(`team1-dev-chatguard-migration`, Terraform 생성)에 `banned_words.txt` 수동 주입(git 미커밋) → chat-server 시작 시 **Flyway V2 마이그레이션**이 읽어 `banned_words`에 `INSERT IGNORE` 적재(**DB 수명당 1회**, `UNIQUE(word)` 멱등). DB는 런타임 진실원 유지(캐시·admin CRUD·`config:banned-words` 무효화 불변). S3 미가독 시 더미 4단어 fallback. S3 접근=IRSA(`DefaultCredentialsProvider`, 정적 키 0). env=`AWS_S3_BUCKET`/`AWS_S3_KEY`/`AWS_REGION`(ConfigMap). | git 커밋(`aws_s3_object`·`.env`·ConfigMap·시드 SQL) — 금칙어 레포 노출 / 매 부팅 truncate+reload | 제품 주제가 **검열**이라 금칙어 목록 자체가 민감정보 — 노출 시 차단어 공개로 우회 용이. ∴ 목록은 git 밖(S3 수동 주입), 진실원(DB)은 그 시드로 부트스트랩. Flyway 1회+`INSERT IGNORE`라 admin CRUD(D46) 보존. IRSA로 장기 자격증명 0(C-1 검증한 SA/role). | 🟢 구현됨·문서 반영. (보강 후보: ① fallback 진입이 stderr 로그뿐·메트릭 부재 → B 관측성에 카운터/알람 ② DB 재생성 시 V2 재시드로 admin이 지운 seed 단어 부활 — dev에선 무해) |
...
> D49는 야간 차단 중 destroy 중단 사고(2026-06-23)에서 실증돼 정해졌고, infra README destroy 절차·config ArgoCD Service 태그 어노테이션·CLAUDE.md §4 체크리스트로 반영 예정이다(앱 본문 A~E 무영향). 상세 경위·복구 절차는 학습 문서 `2026-06-23 destroy 사고 회고` 참조.
> D50은 WS 연결 cap PR(#57, 2026-06-24 머지)에서 AI 리뷰로 드러난 계약 보강이며 A-1·A-3·A-5에 반영됐다. cap 거부 close code=`1013`, 신규 env `WS_CONNECTION_CAP`. **`CHAT_FROZEN`은 이미 A-3 에러표에 존재(D45 반영분)라 추가 변경 없음** — 일부 리뷰가 "누락"으로 본 것은 app 레포의 `_context` DESIGN.md 사본이 6/18 freeze 업데이트 이전 버전이기 때문으로 보이며, 사본 동기화 점검을 권고한다. 미결로 남긴 cap 체크 TOCTOU(pod 200 규모 실용 허용)는 부하테스트 후 재검토.
> D51은 금칙어 S3 시드 구현 검증(2026-06-26, Plan Mode read)에서 코드값으로 확인돼 D42를 보강했고 A-2·A-4·A-5에 반영됐다. 핵심: Flyway V2 1회 시드(`INSERT IGNORE`·`UNIQUE(word)` 멱등)·ConfigMap env 3곳 일치·IRSA 전용·fallback 더미 4단어. **★ 운영 주의: 시드는 DB 수명당 1회라 `banned_words.txt`는 chat-server 첫 부팅 전에 S3에 있어야 한다** — 없으면 더미 4단어로 굳고 Flyway가 V2를 재실행하지 않는다(복구=S3 적재 후 V2 강제 재실행 또는 DB 재생성). 검증 = `banned_words` count > 4 또는 로그 'Successfully seeded ... from S3'.

---

### 결정 기록 — 주말 집중 세션 (2026-06-28 추가)

> 6/27 전략 세션 결정 + 정합성 점검(설계≠구현 갭)을 SSoT에 승격. D9(t-family 고정)를 일부 번복하고(D53), 정합성 점검이 드러낸 prod HA 미구현(D54)·관측성 미완성(D55)·stateful 휘발(D56)을 결정으로 확정한다. 모두 🟢 설계 확정·구현 예정. 점검이 추가로 찾은 코드 버그·드리프트(#1~#12: prod platform-addons 출력 이름 불일치·prod frontend 이미지 override 누락·redis_exporter 버전 오기 등)는 설계 결정이 아니므로 DESIGN엔 싣지 않고 해당 infra/config PR 설명에 기록한다. 개인계정 이식안은 검토 후 폐기(이식 품 과다) → infra 직접 수정 후 월요일 부트캠프 apply로 오류 수렴(마스터플랜 기록, 설계 무영향).

| ID  | 결정 | 채택 | 검토한 대안 | 근거 | 상태 |
| --- | --- | --- | --- | --- | --- |
| D53 | 노드 인스턴스 t→m7g(arm64) (D9 일부 번복) | EKS 노드 dev/prod **m7g.large(arm64)** + `ami_type=AL2023_ARM_64_STANDARD` 동반. 빌드는 **buildx 멀티아치**(`linux/amd64,linux/arm64`; `setup-qemu`+`setup-buildx`+`build-push-action`). c7g(compute)는 worker CPU 집약 특성상 **측정 후보(stretch)** — prod 안정 m7g 먼저, 시간 남으면 c7g 비교. RDS/Redis family는 **t4g 유지**(C-3 주석) | x86 m7i 유지(빌드 무변경이나 DB·Redis t4g/arm과 불일치) / t-family 유지(D9 원안) | D9는 "데모엔 과한 사양 불필요"로 t를 골랐으나, **부하 측정 시 t의 CPU 크레딧 소진이 "앱 한계"를 "크레딧 한계"로 오염**(멘토①, 2차 관측 t-type 그림자). m계열은 크레딧 없어 깨끗한 데이터. arm 선택 근거: ①DB·Redis(t4g/arm) 아키텍처 일관 ②Graviton 단가(~15%) ③torch 2.5.1 linux aarch64 cp311 휠 PyPI 실존 + **로컬 buildx 빌드 88초 실증**(torch 39s·모델 베이크 18s 통과) ④fe(JS)·be(JVM) 칩 무관. `ami_type`·`instance_types`는 반드시 동반 변경 | 🟢 |
| D54 | prod 실 HA화 (코드↔C-3 정합) | prod 한정: **Redis `num_cache_clusters≥2` + `automatic_failover_enabled` + `multi_az_enabled`**(모듈에 변수 신설, 현재 없음) / **NAT AZ별 다중**(현 단일 하드코딩) / **AZ 3**(현 2 하드코딩) / RDS Multi-AZ(이미 true). dev는 비용 위해 단일 유지. 동반: RDS `final_snapshot_identifier` 추가(미설정 시 prod destroy/replace 에러) | 현행(C-3엔 "prod=HA"라 적혀 있으나 코드는 미구현) 유지 | 정합성 점검 실측: 모듈이 Redis 단일노드·NAT 단일·AZ 2를 **하드코딩**해 C-3의 prod HA 명세와 불일치(전형적 설계≠구현). FIS 장애주입(발표 하이라이트)이 "AZ 죽였더니 N초 failover"로 의미 있으려면 실제 다중화가 전제. 검증본 모듈 수정이라 영향범위 큼 → tflint/checkov 집중. **★ 현 단계(prod 미가동·부트캠프 daily-destroy) 한정 편차**: prod `deletion_protection=false`·`skip_final_snapshot=true`(C-3 명세 true/false와 의도적 불일치) — 매일 destroy 가능하게 + 정적 스냅샷명 충돌(`SnapshotAlreadyExistsFault`)·쓰레기 회피. prod 실운영 전환 시 C-3 값(protection=true·skip=false·**유니크** `final_snapshot_identifier`)으로 복구, validation 블록이 이름 누락을 plan에서 차단. Redis HA(2노드+failover+multi_az)는 현 단계에도 적용(FIS 검증 대상) | 🟢 |
| D55 | 관측성 B-3/B-4 완성 + SRE 갭 클로징 | ①고아 Grafana 대시보드(`base/grafana/dashboards/...json`)를 **kustomize 배선**(ConfigMap + `grafana_dashboard` 라벨, 현재 미참조라 ArgoCD 미배포) ②**PrometheusRule 2종**(`HighChatErrorRate`·`ModerationLatencyHigh`, `release` 라벨 일치) 작성 ③(선택)SLO recording rule·번레이트. 현 상태=**scrape-only**(PodMonitor만 존재) | 전체 SRE 스택 신규 구현 / 현행 scrape-only 유지 | 멘토⑦("SRE 준비 부족") + 점검 결과 B-1~B-4가 **설계만**(레포 전체 PrometheusRule 0건·대시보드 고아). 6/17 중간점검 기준(최소 1서비스 스크레이프)은 충족이나 완성본 미구현. config(GitOps)·AWS 무관이라 안전·저비용. AWS 켜기 전 레포측 선작업으로 적합 | 🟢 |
| D56 | stateful state 레이어 분리 | RDS·Redis·ECR·금칙어 S3를 infra 레이어에서 **별도 state 레이어로 분리** → 매일 destroy/apply 시 데이터·금칙어 시드(D51) 보존. **ECR+S3는 network 결합 0 → `state mv`로 즉시 분리 가능**. **RDS+Redis는 network output 4체인(vpc_id·subnet·SG) + EKS/bastion SG-ID cross-state churn 수용이 전제**(무거움, 단계적). 동반: `force_destroy` 변수화(현 모듈 하드코딩 true, prod 미디어 무음삭제 위험)·`deletion_protection`·`prevent_destroy` lifecycle | 현행 단일 infra state(매일 전소) / 개인계정에서 리허설 후 적용 | 매일 destroy 맥락에서 금칙어 시드·DB 데이터가 매번 휘발 = 운영 마찰. 개인계정 리허설안은 이식 품 과다로 폐기 → 부트캠프 직접 적용(리스크는 plan 선검토 + ECR/S3 우선의 단계적 적용으로 흡수). 실무 정석은 별도 검증환경이나 본 프로젝트 제약상 optimistic 채택 | 🟢(단계적) |
| D57 | worker 장애대응 구조 (processing queue + retry + DLQ + recovery) | BRPOP 단순 pop을 **`brpoplpush`(MOD_QUEUE→PROCESSING_QUEUE) + ack/retry/DLQ/startup-recovery**로 보강. job 처리 전 processing 큐로 이동→성공 시 ack(제거)/실패 시 retry_count++ 후 재적재→`MAX_RETRY_COUNT` 초과 시 DLQ→워커 startup 시 stale(>`PROCESSING_TIMEOUT_SECONDS`) processing job 복구. 신규 env 6개(A-5)·메트릭 3종(B-2). v1 그대로 in-process(D3)·Redis List(D1) 유지 | D1(Redis List) 유지·유실 허용 / D11(SIGTERM graceful)만으로 충분 / 큐를 Streams·SQS로 교체 | D11(graceful shutdown)은 *정상 종료* 시 in-flight를 지키나, **SIGKILL·OOM·crash 시엔 processing 중 job이 증발**(BRPOP no-ack의 알려진 구멍 — D11 근거에 명시됨). reliable queue 패턴(processing 큐+recovery)으로 D1을 안 바꾸고 그 구멍을 메움. Streams/SQS 전환(D1 대안)보다 가벼움. ※`brpoplpush`는 Redis 6.2+ deprecated(→`blmove`)·`claim_processing_job` race는 app PR에서 처리 | 🟢 구현(app)·문서 반영 |

> D53은 D9를 일부 번복한다(C-3 표·D9 행 갱신). D54~D56은 정합성 점검(2026-06-28)이 식별한 설계≠구현 갭의 결정화다. D53 arm 전환은 torch aarch64 휠 PyPI 실존 확인 + 로컬 buildx 빌드(88초) 실증으로 뒷받침된다. 마스터플랜의 작업항목 표기(D-0·D-1 등)는 본 결정번호(D1~D52)·부하테스트 절(§D)과 충돌하므로 본 세션 결정은 D53~로 발급하고 작업항목과 분리했다.
