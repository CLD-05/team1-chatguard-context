...
| D54 | prod 실 HA화 (코드↔C-3 정합) | prod 한정: **Redis `num_cache_clusters≥2` + `automatic_failover_enabled` + `multi_az_enabled`**(모듈에 변수 신설, 현재 없음) / **NAT AZ별 다중**(현 단일 하드코딩) / **AZ 3**(현 2 하드코딩) / RDS Multi-AZ(이미 true). dev는 비용 위해 단일 유지. 동반: RDS `final_snapshot_identifier` 추가(미설정 시 prod destroy/replace 에러) | 현행(C-3엔 "prod=HA"라 적혀 있으나 코드는 미구현) 유지 | 정합성 점검 실측: 모듈이 Redis 단일노드·NAT 단일·AZ 2를 **하드코딩**해 C-3의 prod HA 명세와 불일치(전형적 설계≠구현). FIS 장애주입(발표 하이라이트)이 "AZ 죽였더니 N초 failover"로 의미 있으려면 실제 다중화가 전제. 검증본 모듈 수정이라 영향범위 큼 → tflint/checkov 집중 | 🟢 |
